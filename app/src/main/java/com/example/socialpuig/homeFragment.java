...
public class homeFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        class PostsAdapter extends FirestoreRecyclerAdapter<Post, PostsAdapter.PostViewHolder> {
            ...
            protected void onBindViewHolder(@NonNull PostViewHolder holder, int position, @NonNull final Post post) {
                ...
                if (currentUserId.equals(postAuthorId)) {
                    // El usuario actual es el autor del post, permitir eliminar el post
                    holder.deleteImageView.setVisibility(View.VISIBLE);
                    holder.deleteImageView.setOnClickListener(view -> {
                        // Accede al ID de la publicación asociada a este ViewHolder
                        String postId = getSnapshots().getSnapshot(position).getId();

                        // Llama a un método para eliminar la publicación
                        deletePost(postId);
                    });
                } else {
                    // El usuario actual no es el autor del post, ocultar la opción de eliminar
        ...
        // Accede a la colección "posts" y elimina la publicación correspondiente al postId
        db.collection("posts").document(postId)
                .delete()
                .addOnSuccessListener(aVoid -> {
                    // La publicación se eliminó exitosamente
                    // Puedes mostrar un mensaje o realizar cualquier acción adicional aquí si lo deseas
                })
                .addOnFailureListener(e -> {
                    // Ocurrió un error al intentar eliminar la publicación
            ...
            class PostViewHolder extends RecyclerView.ViewHolder {
                ...
                TextView authorTextView, contentTextView, numLikesTextView, dateTextView, numRepostTextView;
